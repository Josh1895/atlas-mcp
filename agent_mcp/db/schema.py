...
def init_database() -> None:
    ...
    try:
        ...
        # --- Swarm Tables (P-006: Persistent Traceability) ---

        # Swarm Runs Table - stores complete run metadata
        cursor.execute(
            """
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_swarm_runs_status ON swarm_runs (status, started_at DESC)"
        )
        logger.debug("Swarm_runs table and indexes ensured.")

        # Swarm Agents Table - individual micro-agents in a run
        cursor.execute(
            """
            CREATE TABLE IF NOT EXISTS swarm_agents (
                ...
                error TEXT,
                PRIMARY KEY (run_id, swarm_agent_id),
                FOREIGN KEY (run_id) REFERENCES swarm_runs(run_id)
            )
        """
        )
        logger.debug("Swarm_agents table ensured.")

        # Swarm Outputs Table - outputs produced by agents
        cursor.execute(
            """
            CREATE TABLE IF NOT EXISTS swarm_outputs (
                ...
                PRIMARY KEY (run_id, swarm_agent_id),
                FOREIGN KEY (run_id, swarm_agent_id) REFERENCES swarm_agents(run_id, swarm_agent_id)
            )
        """
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_swarm_outputs_cluster ON swarm_outputs (run_id, cluster_id)"
        )
        logger.debug("Swarm_outputs table and index ensured.")

        # Tool Cache Table - cached expensive tool results
        cursor.execute(
            """
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_tool_cache_tool ON tool_cache (tool_name, created_at DESC)"
        )
        logger.debug("Tool_cache table and indexes ensured.")

        # Tool Calls Table - audit log of tool invocations
        cursor.execute(
            """
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_tool_calls_run ON tool_calls (run_id, created_at DESC)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_tool_calls_tool ON tool_calls (tool_name, created_at DESC)"
        )
        logger.debug("Tool_calls table and indexes ensured.")
