...
from ..external.gemini_service import initialize_gemini_client
...
async def application_startup(
    ...
    # 6. Initialize Gemini Client (unified AI services under GEMINI_API_KEY)
    # We explicitly initialize it at startup now.
    if not initialize_gemini_client():
        logger.warning(
            "Gemini client failed to initialize. AI-dependent features (like RAG, embeddings) will be unavailable."
        )
        # Server can continue, but AI features won't work.
